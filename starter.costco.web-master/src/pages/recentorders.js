...
const RecentOrders= ()=>{
    ...
    let [endIndex, setEndIndex]= useState(10);
    ...
    useEffect(()=> {
        const items = orderList.filter((data)=>{
            if(search == null)
                return data;
            else if(data.orderNumber.toLowerCase().includes(search.toLowerCase()) || 
    ...
    useEffect(() => { 
        const handleScroll = () =>{
            ...
            if((window.innerHeight + window.scrollY >= (document.body.offsetHeight-100)) 
                && !loading && endIndex<searchOrderList.length){
                setLoading(true);
                setTimeout(()=> {
        ...
        <Layout>
            <Box>
                ...
                {searchOrderList.length>0? <>
                    <TextField label="Search" type="search" onChange={handleSearchChange}/>
                    <OrderList orderList={searchOrderList.slice(0, endIndex)} search={search}></OrderList> 
                    </> :
                    <></>
                }
                {loading?
